...
class ChatAgentContainer(SequentialRoutedAgent):
    ...
    async def handle_error(self, message: GroupChatError, ctx: MessageContext) -> None:
        """Handle a group chat error event. The error is already handled by the orchestrator;
        agent containers simply ignore it to avoid unhandled-message exceptions."""
        pass
